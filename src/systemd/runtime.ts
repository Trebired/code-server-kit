...
async function launchCodeServerWithSystemd(
  options: CodeServerSystemdLaunchOptions,
): Promise<CodeServerSystemdLaunchResult> {
  ...
  if (existing && !existing.notFound) {
    const reusableResult = await handleExistingSystemdUnit(
      existing,
      command,
      options,
    );
  ...
  try {
    ...
    throw new CodeServerSystemdLaunchError(
      "Could not launch code-server with systemd-run.",
      {
        args: command.args,
...
async function restartCodeServerSystemdUnit(
  options: CodeServerSystemdStopOptions,
): Promise<void> {
...
async function stopCodeServerSystemdUnit(
  options: CodeServerSystemdStopOptions,
): Promise<void> {
  const log = resolveLogger(options.logger, options.loggerAdapter);
  ...
  try {
    ...
    if (options.resetFailed) {
      await runSystemCommand("systemctl", [
          scopeFlag,
          "reset-failed",
          options.unitName,
      ]);
    ...
    throw new CodeServerSystemdStatusError(
      "Could not stop the code-server systemd unit.",
      {
        cause: error instanceof Error ? error.message : String(error),
        scope: options.scope,
        unitName: options.unitName,
...
async function readCodeServerSystemdStatus(options: {
  ...
  try {
    const output = await runSystemCommand(
      "systemctl",
      createSystemdStatusArgs(scopeFlag, unitName),
    );
    const status = parseSystemdShowOutput(output, scope, unitName);
    return attachSystemdStatusResult(status);
  } catch (error) {
    throw new CodeServerSystemdStatusError(
      "Could not read the code-server systemd unit status.",
      {
        cause: error instanceof Error ? error.message : String(error),
        scope,
        unitName,
...
function attachSystemdStatusResult(
  status: CodeServerSystemdStatus,
): CodeServerSystemdStatus {
  return {
    ...status,
...
function createSystemdStatusArgs(
  scopeFlag: string,
  unitName: string,
): string[] {
  return [
...
function logReadSystemdStatus(
  ...
  resolveLogger(options.logger, options.loggerAdapter).info(
    "systemd",
    "reading code-server systemd status",
    { scope, unitName },
  );
}

async function readCodeServerSystemdJournal(
  options: CodeServerSystemdJournalOptions,
): Promise<string> {
  const log = resolveLogger(options.logger, options.loggerAdapter);
  const scope = normalizeSystemdScope(options.scope);
  const unitName = normalizeSystemdUnitName(options.unitName);
  ...
  try {
    ...
    throw new CodeServerSystemdJournalError(
      "Could not read the code-server systemd journal.",
      {
        cause: error instanceof Error ? error.message : String(error),
        lines,
        scope,
...
async function summarizeCodeServerSystemdJournal(
  options: CodeServerSystemdJournalOptions,
): Promise<string> {
...
async function extractCodeServerSystemdFailure(
  options: CodeServerSystemdJournalOptions,
): Promise<CodeServerSystemdFailure> {
...
async function safeReadStatus(
  scope: CodeServerSystemdScope,
  unitName: string,
): Promise<CodeServerSystemdStatus|null> {
  try {
