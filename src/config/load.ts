...
async function loadConfig(
  ...
  options: LoadCodeServerKitConfigOptions = {},
): Promise<LoadedCodeServerKitConfig> {
  ...
  if (!await pathExists(configPath)) throw new Error(`code-server-kit config was not found: ${configPath}`);
...
function loadConfigSync(
  ...
  if (!configPath) return handleMissingConfig(options);
  if (!fs.existsSync(configPath)) throw new Error(`code-server-kit config was not found: ${configPath}`);
  return loadedConfig(configPath, readSourceConfig(fs.readFileSync(configPath, "utf8"), configPath));
}

function loadCachedConfigSync(projectRoot = process.cwd()): NormalizedCodeServerKitConfig {
  const root = path.resolve(projectRoot);
...
function resetConfigCacheForTests(): void {
  cachedConfigs = new Map<string, LoadedCodeServerKitConfig>();
}
...
async function findConfig(startDir = process.cwd(), boundaryDir?: string): Promise<string|null> {
  ...
  for (;; ) {
    const candidate = path.join(current, CODE_SERVER_KIT_PROJECT_CONFIG_PATH);
...
function handleMissingConfig(options: LoadCodeServerKitConfigOptions): LoadedCodeServerKitConfig {
  if (options.defaultIfMissing === false) throw new Error("code-server-kit config was not found");
  return missingConfig();
}

function missingConfig(): LoadedCodeServerKitConfig {
...
function readSourceConfig(source: string, configPath: string): CodeServerKitConfig {
  const candidate = runConfigSource(source, configPath);
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) {
    throw new Error(`code-server-kit config must default-export an object: ${configPath}`);
  }
  return candidate as CodeServerKitConfig;
}
...
function runConfigSource(source: string, configPath: string): unknown {
  try {
    ...
    throw new Error(`code-server-kit config failed to load: ${configPath}: ${reason}`);
  }
}
...
function toRuntimeConfigSource(source: string, configPath: string): string {
  ...
  if (runtimeSource === withoutImports || /\bexport\b/u.test(runtimeSource)) {
    throw new Error(`code-server-kit config only supports a default export: ${configPath}`);
  }
